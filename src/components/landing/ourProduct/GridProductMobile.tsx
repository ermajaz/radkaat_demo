...
export default function GridProductMobile() {
  return (
    <section
      className="
    ...
    >
      ...
      <div className="relative flex flex-col gap-4 w-full max-w-[500px] h-full justify-center">
        {ourProducts.slice(0, 3).map((product, index) => (
          <HoverCard key={product.id} product={product} delay={index * 0.1} />
        ))}
      </div>
    </section>
  );
...
/* 🎬 HoverCard Component */
function HoverCard({ product, delay }: { product: any; delay?: number }) {
