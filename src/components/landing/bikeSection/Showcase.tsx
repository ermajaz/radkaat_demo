...
export default function Showcase({
  ...
  return (
  <div className="relative w-full min-h-screen text-white flex flex-col min-h-0">
      {/* Left Tab Sticky */}
      <div className="absolute bg-black left-0 pl-2 top-1/2 -translate-y-2/3 flex flex-col gap-5 items-center z-50">
        {bikes?.map((b) => {
          const isActive = b.name === bike.name;

          return (
            <div key={b.name} className="flex items-center mb-4 last:mb-0">
              {/* Small vertical line */}
              <div
                className={`w-[4px] h-10 mr-2 ${isActive ? "" : "bg-gray-400"}`}
                style={{
                  background: isActive
                    ? `linear-gradient(to bottom, ${b.colors.gradient})`
                    : undefined,
                }}
              />
  ...
  <div className="relative flex flex-1 items-center justify-between px-6 md:px-14 -translate-y-10 overflow-y-auto overflow-x-hidden min-h-0">
        ...
        <div className="w-[200px] flex flex-col gap-8 z-20 ml-10">
          ...
          <AnimatePresence initial={false} mode="wait">
            ...
            >
              <Image
                ...
                width={800}
                height={800}
                className="w-[100%] h-auto max-h-[60vh] md:max-h-[80vh] drop-shadow-2xl"
              />
            </motion.div>
          </AnimatePresence>
